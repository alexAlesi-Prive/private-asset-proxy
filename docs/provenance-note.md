...
1. **Privé's construction is standard/public technique.** The proxy is built by **comparable-company / nearest-neighbour approximation in standardised metric space** - i.e. compare a holding's fundamentals (revenue, EBITDA, net income, margins, leverage, yield, etc.) to traded assets described by the same metrics and take the closest as a weighted basket. Comparables/"multiples" analysis, standardisation (z-scoring), log-scaling of size variables, **Mahalanobis distance**, **Hamada beta unlever/relever**, and nearest-neighbour weighting are all textbook, widely used methods. This is safe to implement in Privé's own code.
...
5. **De-smoothing / NAV roll-forward** (§8 of the methodology) are published academic/industry methods; de-smoothing is a proposed Privé addition, not extracted from the reference.

6. **Backtest return series are simulated in the prototype.** The out-of-sample validation harness (methodology §6) needs a return series per baseline asset. In this prototype those series are **synthetically generated** from a transparent factor model (market + sector + idiosyncratic), seeded deterministically and clearly labelled illustrative - they are **not** licensed market data and make no claim about any real asset's returns. For production the backtest consumes the client's **real** return history from the market-data feed; confirm licensing there, same as for the baseline universe (item 4).

7. **Branding:** all former-employer and legacy product names are scrubbed from Privé's authored artifacts; the method is re-expressed in Privé's own words and public technique. Any client-specific data in the reference material is not reused.
